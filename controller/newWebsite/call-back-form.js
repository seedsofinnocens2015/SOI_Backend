...
const buildLeadSquaredPayload = (formData) => {
  ...
  const callBackDetails = [
    `Preferred Call Time: ${callTime || 'NA'}`,
    `Preferred Date: ${date || 'NA'}`,
    `Reason for Call: ${reason || 'NA'}`,
  ].filter(item => !item.includes('NA')).join(' | ');

  const notesMessage = message 
    ? `${callBackDetails}${callBackDetails ? ' | ' : ''}Message: ${message}`
    : callBackDetails || 'Call back request';
...
const sendNotificationEmail = async (formData, options = {}) => {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;">
      <h2 style="color:#c62828;margin-bottom:8px;">New Call Back Request</h2>
...
const createCallBackRequest = async (req, res) => {
  try {
    ...
    console.error('=== Call back request submission error details ===');
